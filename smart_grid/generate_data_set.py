...
def generate_adj_matrix(points, connect_quantity, unit_vector_length, draw):
    """Generate adjacency matrix and draw connections."""
    points_quantity = len(points)
    adj_matrix = np.zeros((points_quantity, points_quantity), dtype=int)
...
def is_connected(adj_matrix):
    ...
    points_quantity = len(adj_matrix)
...
def generate_data(num_images=1, points_quantity=25, image_width=800, image_length=800, edge=20, point_radius=130, 
    ...
    for i in range(num_images):
        while True:
            ...
            adj_matrix = generate_adj_matrix(points, connect_quantity, unit_vector_length, draw)
        ...
        np.save(os.path.join(matrices_path, f"adj_matrix_{i}.npy"), adj_matrix)
